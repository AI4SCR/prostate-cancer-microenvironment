...
def main(base_dir: Path | None = None, resolution: float = 0.75):
    ...
    markers = [
        ...
        "CD20",  # b-cells
        "CD68",  # macrophages
        "CD11b",  # myeloid cells, macrophages, neutrophils
        "CD66b",  # neutrophils
        # stromal
        "Smooth_muscle_actin",
        "Collagen1",
    ...
    marker_groups = {
        ...
        "endothelial": ["CD31", "CD105"],
        "immune": ["CD45", "CD20", "CD68"],
    ...
    save_dir = base_dir / "02.0_clustering" / "undefined" / f"r-{resolution}"
    ...
    if data_path.exists():
        ...
        immune = immune[
            ~immune.index.get_level_values("group_name").isin(["undefined"])
        ]
        ...
        filter_ = epithelial.index.get_level_values("group_name").str.contains(
            "epithelial"
        )
        ...
        levels = set(cells.index.names) - set(data.index.names)
        index = cells.index.droplevel(list(levels))
        index = index.drop_duplicates(keep=False)
        assert index.duplicated().any() == False

        # remove all cells that are already assigned
        filter_ = ~data.index.isin(index)
        data = data[filter_]
        data = data[markers]
