...
def main(base_dir: Path | None = None, resolution: float = 0.75):
    ...
    save_dir = base_dir / "02.0_clustering" / "endothelial" / f"r-{resolution}"
    ...
    if data_path.exists():
        ...
        # FILTER
        endothelial1 = pd.read_parquet(
            base_dir / "02.0_clustering" / "stromal" / f"r-{1.25}" / "data.parquet"
        )
        filter_ = endothelial1.index.get_level_values("group_name").str.contains(
            "endothelial"
        )
        endothelial1 = endothelial1[filter_]

        endothelial2 = pd.read_parquet(
            base_dir
            / "02.0_clustering"
        ...
        filter_ = endothelial2.index.get_level_values("group_name").str.contains(
            "endothelial"
        )
        endothelial2 = endothelial2[filter_]

        cells = pd.concat((endothelial1, endothelial2))
