...
def main(data_dir: Path | None = None):
    ...
    cols_to_plot = ["os_status", "disease_progr", "gs_grp"]
    ...
    plot_path = save_dir / "figure4a_stacked_barplot.pdf"
    ax.figure.savefig(plot_path, dpi=300, bbox_inches="tight")
    logger.info(f"saved Figure 4a to {plot_path}")
    ...
    props.reset_index().to_parquet(save_dir / "figure4a_patient_composition.parquet")
    mdat.reset_index().to_parquet(save_dir / "figure4a_metadata_with_dendrogram_colors.parquet")
    logger.info(f"saved figure 4a panels and data to {save_dir}")
