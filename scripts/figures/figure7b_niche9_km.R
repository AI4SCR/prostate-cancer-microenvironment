...
results_dir <- file.path(output_figures_dir, "figure7", "niches")
dir.create(results_dir, showWarnings = FALSE, recursive = TRUE)
figures_dir <- file.path(output_figures_dir, "figure7", "niches")
...
cols <- "luminal_CAF1(CD105High)" # niche 9
...
for (col in cols) {
  ...
  # print(p_prog)
  # legacy bug: hardcoded list shared across sibling scripts (this script's
  # own niche happened to be in it, so it wasn't actually broken here, but
  # the condition was still stale/copy-pasted) -- see bugs.md
