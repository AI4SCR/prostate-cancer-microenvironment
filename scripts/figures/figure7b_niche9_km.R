# Reproduce Figure 7b: KM survival/progression by high-vs-low niche 9
# abundance (binary median split).
#
# 1:1 port of the old repo's
# 000_paper/11_niches/113_survival/niche_kaplan_meier_binary.R, trimmed to
# niche 9 only (`cols <- "luminal_CAF1(CD105High)"` instead of looping every
# niche) -- the full 18-niche loop is already the already-validated
# figure6_km_niche6.R (not touched here; that script exists for niche 6/
# Figure 6b specifically). Niche 9 = `luminal_CAF1(CD105High)` per the fixed
...
# niches 6 and 16-18; flagged in open-questions.md.
#
...
# $OUTPUT_FIGURES_DIR/figure7/niche_km/threshold_50/.
...
save_dir <- file.path(output_figures_dir, "figure7", "niche_km")
...
cols <- "luminal_CAF1(CD105High)" # niche 9 only
...
cat("Saved Figure 7b (niche 9) KM panels to", save_dir, "\n")
