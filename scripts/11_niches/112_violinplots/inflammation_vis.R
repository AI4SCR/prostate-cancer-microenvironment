...
df_long$inflammation <-
  factor(df_long$inflammation,
         levels = c("no", "yes"))
...
p_up <- ggplot(
  ...
  labs(
    title = "inflammation niches upregulated in reactive stroma",
    x = "Niche",
...
ggsave(
  p_full,
  filename = file.path(result_dir, "violin_boxplot_inflammation_split.pdf"),
  width = 12,
  height = 6,
  dpi = 300
)
...
ggsave(
  p_full,
  filename = file.path(result_dir, "violin_boxplot_inflammation.pdf"),
  width = 12,
  height = 6,
  dpi = 300
)
