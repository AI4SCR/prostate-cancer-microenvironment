# Reviewer response (Issue 2): does niche 6 (tumorERG+p53+_ProlifLuminal) remain
# associated with outcome once patient-level Gleason grade (gs_grp) is in the
# model, vs a Gleason-grade-only model? Extends figure6_km_niche6.R (KM/binary
# risk-group) and reuses figure4de_cox_hazard_ratio.R's CLR-abundance pipeline,
# applied to niches instead of cell types, so niche 6 enters as a continuous
...
stopifnot("niche 6 column not found in niche composition" = niche_col %in% niche_names)

data <- composition
data[, niche_names] <- clr(as.matrix(data[, niche_names]))
data <- data |> select(pat_id, niche6_clr = all_of(niche_col))
...
for (spec in list(
  ...
  multivariate <- fit_cox(cox_data, "niche6_clr + gs_grp") |> mutate(model = "niche6_plus_gleason")

  results <- bind_rows(univariate, multivariate)
  results$outcome <- event.name
  results$n_patients_gs_grp_dropped <- n_gleason_na

  save.path <- file.path(save_dir, sprintf("figure6_cox_niche6_gleason_%s.csv", label))
  ...
  p <- ggplot(pdat, aes(y = term)) +
    ...
    labs(
      x = "Hazard ratio",
      y = NULL,
      title = paste("Niche 6 + Gleason grade -- multivariate Cox --", event.name),
      subtitle = "Points = HR; bars = 95% CI"
    ) +
    theme_minimal(base_size = 11)
  ggsave(
    filename = file.path(save_dir, sprintf("figure6_cox_niche6_gleason_%s.png", label)),
    plot = p, width = 6, height = 4
  )
}

cat("Saved niche 6 + Gleason Cox results to", save_dir, "\n")
